// Package musics provides access to table musics in database.
package musics
...
func (t table) Insert(tx *sqlx.Tx, data Data) (r Record, err error) {
	...
	err = tx.QueryRowx(`
		INSERT INTO musics (
			artist, title, album, release_date, last_played, rating, description
		)
		VALUES (
			$1, $2, $3, $4, $5, $6, $7
		)
		...
		`,
		data.Artist, data.Title, data.Album, data.ReleaseDate, data.LastPlayed, data.Rating, data.Description).
		StructScan(&data)
	if err != nil {
